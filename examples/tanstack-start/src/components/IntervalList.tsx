...
function StatusCell({ interval }: { interval: Interval }) {
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger className="flex items-center rounded-sm hover:bg-muted transition-colors">
...
const columns: ColumnDef<Interval>[] = [
	...
	},
	{
		...
		cell: ({ row }) => (
			...
			>
				<span className="text-sm font-medium truncate">{row.original.title || "Untitled"}</span>
			</Link>
		),
	},
	{
