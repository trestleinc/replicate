	...
	<div class="editor-container">
		{#if isEditingTitle}
			<input
				...
				class="w-full font-sans text-2xl font-medium text-foreground bg-transparent border-none border-b-2 border-primary p-0 pb-1 leading-tight outline-none"
			/>
		{:else}
			<button
				type="button"
				class="w-full font-sans text-2xl font-medium text-foreground leading-tight cursor-text transition-fast hover:text-primary text-left bg-transparent border-none p-0"
		...
		{#if onPropertyUpdate}
			<div class="flex items-center justify-between gap-4 mt-4 mb-6 pb-4 border-b border-border text-sm">
				<div class="flex items-center gap-3">
					<DropdownMenu.Root>
						<DropdownMenu.Trigger class="flex items-center gap-2 px-2 py-1 hover:bg-muted transition-fast">
									...
									<Tooltip.Root>
										<Tooltip.Trigger>
											<Avatar.Root class="size-7 ring-1 ring-border transition-transform hover:scale-105">
												...
												{/if}
												<Avatar.Fallback class="text-xs font-medium text-white" style="background-color: {user.color}">
