...
<Dialog.Root bind:open onOpenChange={(o) => !o && onclose()}>
	<Dialog.Content
		class="w-[85vw] max-w-[85vw] sm:max-w-[400px] h-auto max-h-[80vh] sm:max-h-[85vh] p-0 gap-0"
	...
	>
		...
		<ScrollArea class="flex-1 sm:max-h-[400px]">
			<div class="p-1">
				<!-- Status Section -->
				<div class="mb-2">
					<div class="px-3 py-1.5 font-mono text-xs font-medium text-muted-foreground uppercase tracking-wider">
						Status
					</div>
					<!-- All statuses option -->
					<button
						...
						class={cn(
							...
							selectedSection === "status" && selectedIndex === 0 && "bg-muted text-foreground border-primary",
							statusFilter === null && !(selectedSection === "status" && selectedIndex === 0) && "text-primary"
						)}
					...
					{#each statusOptions as status, index}
						<button
							...
							class={cn(
								"w-full flex items-center gap-3 py-2 px-3 text-left cursor-pointer",
								"transition-fast hover:bg-muted hover:text-foreground border-l-2 border-transparent",
								selectedSection === "status" && selectedIndex === index + 1 && "bg-muted text-foreground border-primary",
								statusFilter === status && !(selectedSection === "status" && selectedIndex === index + 1) && "text-primary"
							)}
				...
				<div>
					...
					<button
						...
						class={cn(
							"w-full flex items-center gap-3 py-2 px-3 text-left cursor-pointer",
							"transition-fast hover:bg-muted hover:text-foreground border-l-2 border-transparent",
							selectedSection === "priority" && selectedIndex === 0 && "bg-muted text-foreground border-primary",
							priorityFilter === null && !(selectedSection === "priority" && selectedIndex === 0) && "text-primary"
						)}
					...
					{#each priorityOptions as priority, index}
						<button
							...
							class={cn(
								"w-full flex items-center gap-3 py-2 px-3 text-left cursor-pointer",
								"transition-fast hover:bg-muted hover:text-foreground border-l-2 border-transparent",
								selectedSection === "priority" && selectedIndex === index + 1 && "bg-muted text-foreground border-primary",
								priorityFilter === priority && !(selectedSection === "priority" && selectedIndex === index + 1) && "text-primary"
							)}
