<script lang="ts">
	...
	async function initializeCollections() {
		try {
			await Promise.all([intervals.init(intervalsMaterial), comments.init(commentsMaterial)]);
			ready = true;
		} catch (err) {
...
{:else if displayError}
	<div class="flex h-screen items-center justify-center">
		<div class="max-w-md px-4 text-center">
			<div class="mb-4">
				<svg
					class="text-destructive mx-auto h-12 w-12"
					fill="none"
					stroke="currentColor"
					viewBox="0 0 24 24"
				>
...
{:else if enableEncryption && encryptionState === 'locked'}
	<div class="flex h-screen items-center justify-center">
		<div class="max-w-md px-4 text-center">
			...
			<h2 class="mb-2 text-xl font-semibold">Data Encrypted</h2>
			<p class="text-muted-foreground mb-6">Your data is encrypted. Unlock to continue.</p>
			<Button onclick={handleUnlock} class="min-w-[120px]">Unlock</Button>
		</div>
	</div>
{:else if enableEncryption && encryptionState === 'setup'}
	<div class="flex h-screen items-center justify-center">
		<div class="max-w-md px-4 text-center">
			...
			<h2 class="mb-2 text-xl font-semibold">Set Up Encryption</h2>
			<p class="text-muted-foreground mb-6">Protect your data with end-to-end encryption.</p>
			<Button onclick={handleUnlock} class="min-w-[120px]">Set Up</Button>
		</div>
	</div>
{:else}
	<div class="flex h-screen items-center justify-center">
		<div class="text-center">
			<div
				class="border-muted-foreground mx-auto mb-4 h-8 w-8
