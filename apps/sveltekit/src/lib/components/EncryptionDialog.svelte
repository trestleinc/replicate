	...
	function getDialogTitle(action: PendingAction): string {
		switch (action.type) {
			case 'passphrase-setup':
				return 'Create Backup Passphrase';
			case 'passphrase-get':
...
>
	<Dialog.Content class="sm:max-w-md">
		...
		{#if $pendingAction.type === 'recovery-show'}
			<div class="py-4">
				<div
					class="bg-muted rounded-lg p-4 text-center font-mono text-lg
			...
			<Dialog.Footer>
				<Button onclick={handleAcknowledge} class="w-full">I've Saved My Recovery Key</Button>
			</Dialog.Footer>
		{:else}
			<form onsubmit={handleSubmit} class="flex flex-col gap-4 py-4">
				{#if $pendingAction.type === 'passphrase-setup' || $pendingAction.type === 'passphrase-get'}
					<Input
						type="password"
